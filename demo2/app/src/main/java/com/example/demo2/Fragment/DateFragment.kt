...
class DateFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.timedatas?.observe(viewLifecycleOwner) { updatedList ->
            val recyclerViewState = binding.recyclerView.layoutManager?.onSaveInstanceState() // Save scroll position

            val adapter = binding.recyclerView.adapter as? TimestampAdapter
            if (adapter == null) {
                binding.recyclerView.adapter = updatedList?.let {
                    TimestampAdapter(it) { id, timestamp -> viewModel.decreaseTimestamp(id, timestamp) }
                }
            } else {
